    ...
    <div className="bg-[#f8fafb] min-h-screen mt-15 pb-20" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 py-14 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-1">
          ...
          >
            {/* Cost card */}
            <div className="bg-white rounded-xl p-7 border border-slate-100 shadow-sm overflow-hidden relative">
              {/* Decorative accent */}
              <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-[#00A699] to-[#00d4c8]" />

              <div className="flex items-center gap-3 mb-7">
                <div className="w-11 h-11 bg-[#00A699]/10 rounded-2xl flex items-center justify-center">
                  <Wallet className="text-[#00A699]" size={22} />
                </div>
                <h3 className="text-xl font-black text-slate-800">Cost Estimates</h3>
              </div>

              <div className="space-y-4">
                {[
                  ['Category', currentPlace.category || 'Heritage'],
                  ['Admission', currentPlace.entryFee || 'Free'],
                ].map(([label, val]) => (
                  <div key={label} className="flex justify-between items-center py-3 border-b border-slate-100">
                    ...
                    <span className="font-black text-slate-700 capitalize text-sm text-right max-w-[55%] truncate">{val}</span>
                  </div>
                ))}
              </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full mt-7 py-4 bg-[#00A699] text-white rounded-2xl font-black hover:bg-[#008d82] transition-colors tracking-wide shadow-lg shadow-[#00A699]/25"
              >
                Plan Trip Itinerary
              </motion.button>
            </div>
            ...
            {/* Quick facts strip */}
            {(currentPlace.bestTime || currentPlace.timings) && (
              <div className="bg-white rounded-xl p-6 border border-slate-100 shadow-sm space-y-4">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Quick Facts</p>
                {currentPlace.bestTime && (
                  <div className="flex items-start gap-3">
                    <ShieldCheck size={16} className="text-[#00A699] mt-0.5 shrink-0" />
                    <div>
                      <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Best Season</p>
                      <p className="text-sm font-black text-slate-700">{currentPlace.bestTime}</p>
                    </div>
                  </div>
                )}
                {currentPlace.timings && (
                  <div className="flex items-start gap-3">
                    <Clock size={16} className="text-[#00A699] mt-0.5 shrink-0" />
                    <div>
                      <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Open Hours</p>
                      <p className="text-sm font-black text-slate-700">{currentPlace.timings}</p>
                    </div>
                  </div>
                )}
              </div>
